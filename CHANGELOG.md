...
- **`capsule verify` checks the stored document too.** The CLI loads JSON records with `Capsule.from_sealed_dict`, hashes the stored document at the `full` and `signatures` levels, and exports records as they were stored.
- **A Capsule changed in memory after it was read fails, including nested changes.** Below the top level the model and its stored document must hold the same keys, and a boolean never equals a number. `CapsuleChain.verify` (and `verifyChain` in TypeScript) reports content that cannot be hashed as a failed record instead of raising.
